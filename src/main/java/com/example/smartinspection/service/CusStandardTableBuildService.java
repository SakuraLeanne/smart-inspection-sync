...
public class CusStandardTableBuildService {
    ...
    private static class SqlHolder {
        ...
        private static final String BUILD_STD_WORK_ORDER = "INSERT INTO cus_std_work_order (" +
            ...
            "SELECT 'property',cs.source_id,cs.voucher_no,cs.region_id,NULL,cs.organization_item_id,NULL,COALESCE(NULLIF(TRIM(cs.place),''),NULLIF(TRIM(cs.address),''))," +
            "cs.customer_id,cs.customer_name,cs.phone,cs.service_source,CASE WHEN cs.service_source=1 THEN '自检自查' ELSE CONCAT('来源-',IFNULL(cs.service_source,'NULL')) END," +
            "cs.service_type,CASE WHEN cs.service_type=0 THEN '报修' ELSE CONCAT('服务类型-',IFNULL(cs.service_type,'NULL')) END,cs.repair_status," +
            "CASE cs.repair_status WHEN 1 THEN '已分配' WHEN 3 THEN '已处理' WHEN 5 THEN '已关闭' ELSE CONCAT('状态-',IFNULL(cs.repair_status,'NULL')) END,cs.dispatch_type_id,NULL,NULL,NULL,cs.dispatch_priority_id,NULL," +
            "cs.accepted_date,cs.accepted_date,cs.appointment_time,cs.finish_time_required,cs.dispatch_time,cs.start_process_time,cs.dispatch_finish_time,cs.confirm_time,cs.details," +
            ...
            "cs.comment,CASE cs.comment WHEN 0 THEN '好评' WHEN 1 THEN '中评' WHEN 2 THEN '差评' ELSE NULL END,cs.comment_content,cs.comment_date,IFNULL(cs.comment_by_customer,0)," +
            ...
            "CASE WHEN cs.service_type=0 THEN 1 ELSE 0 END,CASE WHEN cs.repair_status=5 THEN 1 ELSE 0 END,CASE WHEN cs.repair_status IN (3,5) THEN 1 ELSE 0 END,1,cs.sync_time " +
